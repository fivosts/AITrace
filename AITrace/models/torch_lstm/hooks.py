...
# from samplers import validation_database
...
class tensorMonitorHook(object):
      ...
      if key in self.epoch_tensors and "num_" not in key and not "val_" in key:
        ...
        if isinstance(value, list) or isinstance(value, tuple):
          for idx, _ in enumerate(value):
            self.epoch_tensors[key][idx] += value[idx]
        else:
          self.epoch_tensors[key] += value
      else:
        if isinstance(value, list) or isinstance(value, tuple):
          self.epoch_tensors[key] = []
          for idx, _ in enumerate(value):
            self.epoch_tensors[key].append(value[idx])
        else:
          self.epoch_tensors[key] = value
  ...
  def _logTensors(self):
    ...
    if self.average is True:
      epoch_tensors = (self.epoch_tensors if effective_step == 0
                     else {k: (([x / self.step_freq for x in v] if isinstance(v, list) or isinstance(v, tuple) else v / self.step_freq) if not "num_" in k and not "val_" in k else v) for k, v in self.epoch_tensors.items()})
    else:
      epoch_tensors = (self.epoch_tensors if effective_step == 0
    ...
    for key, value in epoch_tensors.items():
      ...
      if isinstance(value, list) or isinstance(value, tuple):
        for idx, _ in enumerate(value):
          if idx >= len(self.plot_tensors[key]['value']):
            self.plot_tensors[key]['value'].append([])
          self.plot_tensors[key]['value'][idx].append(value[idx])
  ...
  def _tensor2plot(self):
    for (key, value) in self.plot_tensors.items():
      if key != "step":
        if isinstance(value['value'][0], list) or isinstance(value['value'][0], tuple):
